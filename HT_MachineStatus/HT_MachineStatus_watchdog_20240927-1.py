...
class Watcher(FileSystemEventHandler):
    ...
    def update_daily_file(self, df, machine_name, data_dict):
        ...
        if os.path.isfile(daily_file_name):
            existing_df = pd.read_csv(daily_file_name, encoding='utf-8-sig')

            # Convert 'Datetime' to datetime objects in existing_df
            existing_df['Datetime'] = pd.to_datetime(existing_df['Datetime'], errors='coerce')

            # Check for duplicates based on 'Tester' and 'Datetime'
            is_duplicate = ((existing_df['Tester'] == data_dict['Tester']) & (existing_df['Datetime'] == data_dict['Datetime'])).any()
            if not is_duplicate:

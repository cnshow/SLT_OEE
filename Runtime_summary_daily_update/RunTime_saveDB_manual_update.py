...
# v2.0  2024.07.29  CN.Wang   Change to auto mode, daily save runtime data to database dbo.ntcent_runtime
# v2.1  2024.07.30  CN.Wang   fix the bug of Start/Stop time setting
# v2.2  2024.09.30  CN.Wang   save a manual update version 
...
# Show all deprecation warnings for SQLAlchemy
...
#image = Image.open('img/hold.jpg')
#c1.image(image, width=80)
#st.title("SLT Tester Status")

# 設置起始日期為當前日期
#stop_date = date.today()  # auto calculate today, actual data is yesterday
stop_date = date(2024, 9, 28)   # actucal data is for 2024-09-27 runtime

# 計算結束日期
start_date = stop_date - relativedelta(days=1)
period = stop_date - start_date
period_days = period.days
#st.sidebar.write(f"{start_date} ~ {stop_date} , {period_days} days")
period_min = period_days * 24 * 60
sampling_rate = 2

#print(f"{start_date} ~ {stop_date} , {period_days} days")
...
# Ensure df, df_MTK, and df_NVT are not views
df = df.copy()
df_MTK = df_MTK.copy()
df_NVT = df_NVT.copy()

# Replace inf and -inf with NaN using .loc
df.loc[:, 'runrate'] = df['runrate'].replace([np.inf, -np.inf], np.nan)
df_MTK.loc[:, 'runrate'] = df_MTK['runrate'].replace([np.inf, -np.inf], np.nan)
df_NVT.loc[:, 'runrate'] = df_NVT['runrate'].replace([np.inf, -np.inf], np.nan)
...
#st.markdown(f"#### Total : {len(df_results[0])} testers")
#st.write(df_results[1])

#st.markdown(f"#### MTK : {len(df_MTK_results[0])} testers")
#st.write(df_MTK_results[1])

#st.markdown(f"#### NVT : {len(df_NVT_results[0])} testers")
#st.write(df_NVT_results[1])

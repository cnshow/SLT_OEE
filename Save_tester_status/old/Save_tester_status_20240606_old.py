...
# Current time and related calculations
...
for tester, group in grouped:
    ...
    data_to_insert.append([current_time_str, Ndate, Sdate, Shift, customer, tester, status, step, maxsites, runsiteqty, device, family])  # Assuming RowID is auto-incremented or not needed
    
    # Prepare the formatted string
    log = f"{current_time} {Ndate} {Sdate} {Shift} {customer} {tester} {status} {step}  {maxsites} {runsiteqty} {device} {family}"
    print(log)

# Create a DataFrame with the correct structure for insertion
df_to_insert = pd.DataFrame(data_to_insert, columns=['current_time', 'Ndate', 'Sdate', 'Shift', 'customer', 'tester', 'status', 'step', 'maxsites', 'runsiteqty', 'device', 'family'])

# Use SQLAlchemy to insert the DataFrame into the database
df_to_insert.to_sql('ntcent_status', con=engine, schema='dbo', index=False, if_exists='append')

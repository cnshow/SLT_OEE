...
# 讀入的資料表:
# - ntcent (通過 Tester_data SQL 查詢)
# - ntcent_layout
# - oee_MTK_Family
# - ntcent_attr
# - ntcent_HT_MachineStatus

# 寫入的資料表:
# - ntcent_status_test

# 程式邏輯重點:
# 1. 從多個數據源獲取測試機狀態、佈局、設備類型等信息
# 2. 處理和合併數據，計算各種狀態指標
# 3. 處理特殊狀態（如 IDLE, ENG, SETUPW）的時間限制
# 4. 計算 HT 狀態相關的數量
# 5. 確定當前班次和班級
# 6. 準備並插入數據到 ntcent_status_test 表
...
df_to_insert.to_sql('ntcent_status_test', con=engine, schema='dbo', index=False, if_exists='append', dtype={
  'maxsites': Integer,
  'runsiteqty': Integer,
  'HT_M_qty': Integer,
  'HT_A_qty': Integer,
